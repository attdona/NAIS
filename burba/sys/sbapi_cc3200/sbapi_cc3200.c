...
/**
 * @brief the sbapi thread function
 */
static void *_sbapi_loop(void *arg);

/**
 * @brief the sbapi task handle
 */
typedef struct sbapi_t {
...
sbapi_t sbapi = { .main_pid = KERNEL_PID_UNDEF, .ssid = NULL, .password = NULL };

/**
 * @brief   memory segment for the NBAPP thread's stack
 */
#if ENABLE_DEBUG
static char _stack[SBAPI_STACK_SIZE + THREAD_EXTRA_STACKSIZE_PRINTF];
#else
static char _stack[SBAPI_STACK_SIZE];
#endif
...
int sbapi_init(uint32_t options) {
    ...
    sbapi.options = options;

    /* check if thread is already running */
    if (sbapi.main_pid == KERNEL_PID_UNDEF) {
        ...
        /* start SBAPI thread */
        sbapi.main_pid = thread_create(_stack, sizeof(_stack), SBAPI_PRIO,
                                       THREAD_CREATE_STACKTEST, _sbapi_loop,
                                       &parent, "sbapi");
    ...
    return sbapi.main_pid;
}

int sbapi_stop(void) {
    int sts;
    sbapi.main_pid = KERNEL_PID_UNDEF;
...
uint32_t net_atoi(const char* name) {
    ...
    sts = sl_NetAppDnsGetHostByName((signed char *) name, strlen(name), &ip,
                                    SL_AF_INET);
    if (sts < 0) {
        // TODO
    }
...
static void *_sbapi_loop(void *arg) {
    ...
    sbapi.main_pid = thread_getpid();
    sbapi.sig_tim.callback = retry_connection;

    /* initialize message queue */
    msg_init_queue(msg_queue, SBAPI_MSG_QUEUE_SIZE);

    xtimer_set(&sbapi.sig_tim, IP_ACQUIRED_FAST_TIMEOUT);
        ...
        switch (msg.type) {
            ...
            case SMARTCONFIG_ACTIVE:
                if (!IS_IP_ACQUIRED(nwp.status)) {
                    xtimer_set(&sbapi.sig_tim, IP_ACQUIRED_SLOW_TIMEOUT);
                }
                break;
            case AUTO_CONNECTION_TIME_SLOT:
                if (counter == MAX_CONN_COUNTER_VAL) {
                    ...
                    if (sbapi.options & SBAPI_SMARTCONFIG) {
                        LOG_INFO(
                        ...
                        sbapi.sig_tim.callback = smartconfig_still_active;
                    }
                    xtimer_set(&sbapi.sig_tim, IP_ACQUIRED_SLOW_TIMEOUT);
                }
            ...
            case IP_ACQUIRED:
                ...
                msg.type = SBAPI_IP_ACQUIRED;
                bocia_loop_init();
                msg_send(&msg, parent_thr);
                break;
            default:
                DEBUG("sbapi: unknown message\n");
                break;
        }
    }
