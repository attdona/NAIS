...
void uart_remote(const uint8_t *data __attribute__((unused)),
...
void bocia_tcp_accept(bocia_channel_t *ch) {
    ...
    if (sock > 0) {
        bocia_register_handlers(sock, ch->target_pid, bocia_tcp_receive,
                                bocia_tcp_send);
    } else {
...
int16_t bocia_tcp_connect(const char *hostname, uint16_t server_port) {
    ...
    sts = sl_NetAppDnsGetHostByName((signed char *)hostname,
                                    strlen((const char *)hostname), &server_ip,
                                    SL_AF_INET);
